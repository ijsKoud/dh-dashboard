...
export const BotGuide: React.FC = () => {
	return (
		<div className="installation-guide">
			...
			<p className="installation__guide-guide">
				To install everything, run "npm install" in the terminal. If you use yarn as package manager, run "yarn install". If an error pops up
				saying something about node-gyp or node-pre-gyp, make sure to re-install NodeJS and select the optional installations, these are
				required because some packages need to use it.
			</p>
			<p className="installation__guide-guide">
			...
			<p className="installation__guide-guide">
				After creating a bot account, rename the ".env.example" file in the bot folder to ".env". Replace everything at TOKEN in " " with your
				bot token. If you want to get error notifications, create a webhook in a channel you can see and copy the link. Paste that link in the
				" " of LOGS.
			</p>
			<p className="installation__guide-guide">
				Do this for every other variable in the file. The text in the " " should explain you what they are meant for and how to obtain the
				value. For the database, make sure to replace the {"<username>"} with the username of the postgres account, {"<password>"} with the
				password to connect to the database and {"<database name>"} with the database name. If you do not know how to create a postgresql
				database, please check <a href="https://www.youtube.com/watch?v=BLH3s5eTL4Y">this tutorial for windows</a> and{" "}
				<a href="https://www.youtube.com/watch?v=9lq74SafVcw">this tutorial for ubuntu</a>.
			</p>
			<p className="installation__guide-guide">
				After everything is installed and every env value is changed, run "npm run prisma-install" or "yarn run prisma-install" to get prisma
			...
			<img src="/assets/guides/api-config.png" title="Api config" onClick={() => window.open("/assets/guides/api-config.png")} />

			<h5 className="installation__guide-guide">Automod config</h5>
			<img src="/assets/guides/automod-config.png" title="Automod config" onClick={() => window.open("/assets/guides/automod-config.png")} />
			...
			<img src="/assets/guides/ticket-config.png" title="Ticket config" onClick={() => window.open("/assets/guides/ticket-config.png")} />
		</div>
	);
};
